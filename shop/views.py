...
class ShopAPIView(APIView):
    ...
    def get(self, request):
        '''Get logged in merchant organizations '''
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({'message': 'Organizations added sucessfully'}, status=status.HTTP_201_CREATED)
...
class ShopDetailView(APIView):
    ...
    def get(self, request, uid, format=None):
        '''Get detail of organizations'''
        shop = self.get_object(uid)
        if shop.merchant.uid != request.user.uid:
            return Response({'message': 'You are not authorized to view this organizations'}, status=status.HTTP_403_FORBIDDEN)
    ...
    def put(self, request,uid, format=None):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response({"msg":"Organizations Updated Sucessfully"}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def delete(self, request,uid, format=None):
        ...
        shop = self.get_object(uid)
        shop.delete()
        return Response({"msg":"Organizations Successfully Removed"},status=status.HTTP_204_NO_CONTENT)
...
class ShopStatusView(APIView):
    ...
    def post(self, request, uid, status):
        '''Activate/Deactivate organizations'''
        try:
            shop = Shop.objects.get(uid=uid)
        except Shop.DoesNotExist:
            return Response({'message': 'Organizations not found '}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def activate_shop(self, request, shop):
        if Shop.objects.filter(merchant=request.user, is_active=True).exists():
                return Response({"msg": "You already have an active organization. Please log out from that first."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid():
            if shop.active_code == serializer.validated_data['active_code']:
                shop.is_active = True
                shop.save()
                return Response({"msg": "Organization Successfully Activated"}, status=status.HTTP_201_CREATED)
            else:
